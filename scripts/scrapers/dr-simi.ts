/**
 * Scraper Dr. Simi — VTEX (account: farmaciasdeldrsimicl)
 * API verificada y funcionando.
 */
import type { PrecioScrapeado } from "./types.js";

const VTEX_ACCOUNT = "farmaciasdeldrsimicl";
...
export async function scrapeDrSimi(query: string): Promise<PrecioScrapeado[]> {
  ...
  if (!res.ok) {
    console.warn(`[dr-simi] HTTP ${res.status} para "${query}"`);
    return [];
  }
  ...
  return products
    .map((p) => {
      ...
      return {
        ...
        url: `https://www.drsimi.cl${p.link}`,
        cadena: CADENA,
